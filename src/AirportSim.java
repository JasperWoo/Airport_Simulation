	...
	private void initialize() {
		...
		btnAddAirport.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//Airport newAirport = new Airport("LAX", 0.1, 0.1, 0.1, 20, 10, 5, true, -118.4, 33.9); //Los Angelas
				Airport newAirport = new Airport(newAirport_name, newAirport_RuntimeToLand,newAirport_Requiredtimeonground,
						newAirport_RunwayTimeToTakeoff, newAirport_groundCapacity, newAirport_airCapacity, 
						newAirport_NumberOfRunway, IFsupportA380, newAirport_Longitude, newAirport_Latitude);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				AirportEvent newDepartureEvent = new AirportEvent(newairplaneDelayTime, selectedAirport, AirportEvent.PLANE_DEPARTS, newAirplane, 0, 0);
		...
		btnNewButton.setBounds(510, 228, 117, 29);
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < airportArrayList.size(); i++) {
		            for (int j = i; j < airportArrayList.size(); j++) {
		                ...
		                distanceMatrix[i][j] = distanceMatrix[j][i] = 3959.0 * Math.acos(Math.sin(Math.toRadians(airportArrayList.get(i).getM_Lat())) *
		                        Math.sin(Math.toRadians(airportArrayList.get(j).getM_Lat())) +
		                        Math.cos(Math.toRadians(airportArrayList.get(i).getM_Lat())) * Math.cos(Math.toRadians(airportArrayList.get(j).getM_Lat()))
		                                * Math.cos(Math.toRadians(airportArrayList.get(i).getM_Long() - airportArrayList.get(j).getM_Long())));
		            }
		        }
